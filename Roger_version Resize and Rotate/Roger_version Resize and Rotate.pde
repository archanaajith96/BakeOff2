...
void draw() {
  ...
  if (checkForLocation() == false) {
    text("move the square to center", width / 2, inchesToPixels(1.5f));
  }
  if (checkForRotation() == false) {
    text("rotate the square", width / 2, inchesToPixels(1.8f));
    //text("CCW", width / 2 + inchesToPixels(2f), inchesToPixels(1.8f));
    text("CW", width / 2 + inchesToPixels(2.5f), inchesToPixels(1.8f));
    text(degreeDif(), width / 2 + inchesToPixels(3f), inchesToPixels(1.8f));
  }
  if (checkForSize() == false) {
    text("rescale the square", width / 2, inchesToPixels(2.8f));
    text("-", width / 2 + inchesToPixels(2f), inchesToPixels(2.8f));
    text("+", width / 2 + inchesToPixels(2.5f), inchesToPixels(2.8f));
    text(sizeDif(), width / 2 + inchesToPixels(3f), inchesToPixels(2.8f));
  }
...
void scaffoldControlLogic()
{
  ...
  //text("CW", width / 2 + inchesToPixels(2.5f), inchesToPixels(1.8f));
  if (mousePressed && dist( width / 2 + inchesToPixels(2.5f), inchesToPixels(1.8f), mouseX, mouseY)<inchesToPixels(.5f))
    screenRotation++;

  //lower left corner, decrease Z
  //text("-", width / 2 + inchesToPixels(2f), inchesToPixels(2.8f));
  if (mousePressed && dist(width / 2 + inchesToPixels(2f), inchesToPixels(2.8f), mouseX, mouseY)<inchesToPixels(.5f))
    screenZ-=inchesToPixels(.02f);

  //lower right corner, increase Z
  //text("+", width / 2 + inchesToPixels(2.5f), inchesToPixels(2.8f));
  if (mousePressed && dist(width / 2 + inchesToPixels(2.5f), inchesToPixels(2.8f), mouseX, mouseY)<inchesToPixels(.5f))
    screenZ+=inchesToPixels(.02f);
